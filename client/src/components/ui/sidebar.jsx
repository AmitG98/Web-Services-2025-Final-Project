  ...
  (
    ...
    const setOpen = React.useCallback(
      (value) => {
        ...
        document.cookie = `${COOKIE_NAME}=${nextState}; path=/; max-age=${COOKIE_MAX_AGE}`;
      },
      [setOpenProp, open]
    );
    ...
    React.useEffect(() => {
      const handleKeyDown = (event) => {
        const isShortcut = event.key === KEYBOARD_SHORTCUT && (event.metaKey || event.ctrlKey);
        if (isShortcut) {
  ...
  (
    ...
    return (
      <Tooltip>
        ...
        <TooltipContent side="right" align="center" hidden={hidden} {...tooltipProps} />
      </Tooltip>
    );
  }
);
